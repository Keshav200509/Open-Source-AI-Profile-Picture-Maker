...
router.post('/:jobId', (req: Request, res: Response) => {
  ...
  res.status(202).json({ success: true, data: { message: 'Background removal started' } });

  const inputPath = getFilePath(job.id, job.resultFile ?? job.originalFile);
  const outputPath = getFilePath(job.id, 'result.png');

  removeBackground(inputPath, outputPath)
    .then(() => updateJob(job.id, { status: 'completed', resultFile: 'result.png' }))
